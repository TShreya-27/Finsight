...
def set_default_model(agent: Agent) -> None:
    ...
    if agent.model is None:
        try:
            from agno.models.groq import OpenAIResponses
        except ModuleNotFoundError as e:
            log_exception(e)
            raise ImportError(
                "Agno agents use `Groq` as the default model provider. Please provide a `model` or install `openai`."
            ) from e

        log_info("Setting default model to Groq Responses")
