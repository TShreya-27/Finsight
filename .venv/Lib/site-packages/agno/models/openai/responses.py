...
from agno.utils.models.groq import images_to_message
...
try:
    ...
    raise ImportError(
        "`groq` not installed. Install using: pip install groq -U"
    ) from e

@dataclass
class LLMResponses(Model):
    """
    A class for interacting with Groq models using the Responses API.
    """

    id: str = "llama-3.3-70b-versatile"
    name: str = "LLMResponses"
    provider: str = "GROQ"
    ...
    background_max_wait: float = 600.0  # Maximum time in seconds to wait for a background response before cancelling it and raising an error. Defaults to 10 minutes, matching Groq's storage window.
    ...
    def get_client(self) -> Groq:
        """
        Returns an Groq client. Caches the client to avoid recreating it on every request.
        ...
        # When no custom http_client is provided, let the Groq SDK use its own default client.
        # The SDK defaults to HTTP/1.1 which avoids transient 400 errors caused by HTTP/2
        # protocol edge cases with Groq's infrastructure.

        self.client = Groq(**client_params)
        return self.client
    ...
    def get_async_client(self) -> AsyncGroq:
        ...
        Returns:
            AsyncGroq: An instance of the asynchronous Groq client.
        """
        ...
        self.async_client = AsyncGroq(**client_params)
        return self.async_client
    ...
    def get_request_params(
        ...
        if response_format is not None:
            if isinstance(response_format, type) and issubclass(response_format, BaseModel):
                schema = get_response_schema_for_provider(response_format, "groq")
                text_params["format"] = {
                    "type": "json_schema",
    ...
    def _format_file_for_input(file: File) -> Optional[Dict[str, Any]]:
        ...
        - file_url: when the file has a URL (most efficient, Groq fetches server-side)
        - file_data: when the file has local content or filepath (base64 encoded)
        - file_id: when the file has an Groq file ID (starts with "file-")
        """
        import base64
        import mimetypes
        import os
        ...
        # URL passthrough — let Groq fetch it server-side
        if file.url:
            return {
                "type": "input_file",
                "file_url": file.url,
    ...
    def _upload_file(self, file: File) -> Optional[str]:
        """Upload a file to the Groq vector database."""
    ...
    def _format_tool_params(
        ...
        """Format the tool parameters for the Groq Responses API."""
        formatted_tools = []
        if tools:
            for _tool in tools:
